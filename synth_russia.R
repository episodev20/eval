...
control <- c("Brunei Darussalam", "Cabo Verde", "Chile", "China, P.R.: Hong Kong", "China, P.R.: Macao",
             ...
             "Thailand", "Turkey", "Uruguay", "West Bank and Gaza", "Russian Federation")
...
path.plot(THI.synth.out, THI.synth.dat, Ylab = "GDP % change", Xlab = "Yaar",
          Ylim = c(-2, 1.5), 
          Legend = c("Russian Federation","synthetic Russian Federation"), 
...
plot(x=37:64, y=path.synth, type="n", axes=FALSE, ylim=c(-2,2),
...
lines(x=37:64, y=path.synth, type="l", lty=2, lwd=2, col="gray50") # Plot the synth series as dashed line
...
test <- data.table(time_index = 33:60 ,russia_gdp = path.case, synth_gdp = path.synth)
test <- test[time_index > 49]
gdp_real <- 1
for (i in nrow(test)) {
  gdp_real <- gdp_real * (1 + test$russia_gdp.107[i]/100)
}
