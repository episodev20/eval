...
#addopting exchange rate table for EURO zone coutnries
new_line <- cbind(data.table(Country = "Austria"), Exchange_Rates[Country == "Euro Area", 2:343])
Exchange_Rates <- rbind(Exchange_Rates, new_line)

new_line <- cbind(data.table(Country = "Belgium"), Exchange_Rates[Country == "Euro Area", 2:343])
Exchange_Rates <- rbind(Exchange_Rates, new_line)

replacement <-which(is.na(Exchange_Rates[Country == "Cyprus"]))
new_line <- cbind(Exchange_Rates[Country == "Cyprus", .SD, .SDcols= !replacement], Exchange_Rates[Country == "Euro Area", .SD, .SDcols= replacement])
Exchange_Rates <- Exchange_Rates[!Country == "Cyprus",]
Exchange_Rates <- rbind(Exchange_Rates, new_line)

replacement <-which(is.na(Exchange_Rates[Country == "Estonia, Rep. of"]))
new_line <- cbind(Exchange_Rates[Country == "Estonia, Rep. of", .SD, .SDcols= !replacement], Exchange_Rates[Country == "Euro Area", .SD, .SDcols= replacement])
Exchange_Rates <- Exchange_Rates[!Country == "Estonia, Rep. of",]
Exchange_Rates <- rbind(Exchange_Rates, new_line)

new_line <- cbind(data.table(Country = "Finland"), Exchange_Rates[Country == "Euro Area", 2:343])
Exchange_Rates <- rbind(Exchange_Rates, new_line)

new_line <- cbind(data.table(Country = "France"), Exchange_Rates[Country == "Euro Area", 2:343])
Exchange_Rates <- rbind(Exchange_Rates, new_line)

new_line <- cbind(data.table(Country = "Germany"), Exchange_Rates[Country == "Euro Area", 2:343])
Exchange_Rates <- rbind(Exchange_Rates, new_line)

replacement <-which(is.na(Exchange_Rates[Country == "Greece"]))
new_line <- cbind(Exchange_Rates[Country == "Greece", .SD, .SDcols= !replacement], Exchange_Rates[Country == "Euro Area", .SD, .SDcols= replacement])
Exchange_Rates <- Exchange_Rates[!Country == "Greece",]
Exchange_Rates <- rbind(Exchange_Rates, new_line)

new_line <- cbind(data.table(Country = "Ireland"), Exchange_Rates[Country == "Euro Area", 2:343])
Exchange_Rates <- rbind(Exchange_Rates, new_line)

new_line <- cbind(data.table(Country = "Italy"), Exchange_Rates[Country == "Euro Area", 2:343])
Exchange_Rates <- rbind(Exchange_Rates, new_line)

replacement <-which(is.na(Exchange_Rates[Country == "Latvia"]))
new_line <- cbind(Exchange_Rates[Country == "Latvia", .SD, .SDcols= !replacement], Exchange_Rates[Country == "Euro Area", .SD, .SDcols= replacement])
Exchange_Rates <- Exchange_Rates[!Country == "Latvia",]
Exchange_Rates <- rbind(Exchange_Rates, new_line)

replacement <-which(is.na(Exchange_Rates[Country == "Lithuania"]))
new_line <- cbind(Exchange_Rates[Country == "Lithuania", .SD, .SDcols= !replacement], Exchange_Rates[Country == "Euro Area", .SD, .SDcols= replacement])
Exchange_Rates <- Exchange_Rates[!Country == "Lithuania",]
Exchange_Rates <- rbind(Exchange_Rates, new_line)

new_line <- cbind(data.table(Country = "Luxembourg"), Exchange_Rates[Country == "Euro Area", 2:343])
Exchange_Rates <- rbind(Exchange_Rates, new_line)

replacement <-which(is.na(Exchange_Rates[Country == "Malta"]))
new_line <- cbind(Exchange_Rates[Country == "Malta", .SD, .SDcols= !replacement], Exchange_Rates[Country == "Euro Area", .SD, .SDcols= replacement])
Exchange_Rates <- Exchange_Rates[!Country == "Malta",]
Exchange_Rates <- rbind(Exchange_Rates, new_line)

new_line <- cbind(data.table(Country = "Netherlands, The"), Exchange_Rates[Country == "Euro Area", 2:343])
Exchange_Rates <- rbind(Exchange_Rates, new_line)

new_line <- cbind(data.table(Country = "Portugal"), Exchange_Rates[Country == "Euro Area", 2:343])
Exchange_Rates <- rbind(Exchange_Rates, new_line)

replacement <-which(is.na(Exchange_Rates[Country == "Slovak Rep."]))
new_line <- cbind(Exchange_Rates[Country == "Slovak Rep.", .SD, .SDcols= !replacement], Exchange_Rates[Country == "Euro Area", .SD, .SDcols= replacement])
Exchange_Rates <- Exchange_Rates[!Country == "Slovak Rep.",]
Exchange_Rates <- rbind(Exchange_Rates, new_line)

replacement <-which(is.na(Exchange_Rates[Country == "Slovenia, Rep. of"]))
new_line <- cbind(Exchange_Rates[Country == "Slovenia, Rep. of", .SD, .SDcols= !replacement], Exchange_Rates[Country == "Euro Area", .SD, .SDcols= replacement])
Exchange_Rates <- Exchange_Rates[!Country == "Slovenia, Rep. of",]
Exchange_Rates <- rbind(Exchange_Rates, new_line)

new_line <- cbind(data.table(Country = "Spain"), Exchange_Rates[Country == "Euro Area", 2:343])
Exchange_Rates <- rbind(Exchange_Rates, new_line)

#replace NAs with 1, so that when multiplyied, to effect on currency 
#(supposed for EU contries adopting euro)
# but need to check if there are other NAs with other reasoning.

#Exchange_Rates[is.na(Exchange_Rates)] <- 1


treatment <- c("Austria", "Belgium", "Bulgaria", "Croatia, Rep. of", "Cyprus", "Czech Rep.", "Denmark",
               "Estonia, Rep. of", "Finland", "France", "Germany", "Greece", "Hungary", "Ireland",
               "Italy", "Latvia", "Lithuania", "Luxembourg", "Malta", "Netherlands, The", 
               "Poland, Rep. of", "Portugal", "Romania", "Slovak Rep.", "Slovenia, Rep. of", 
...
# unify countries in treatment
new_line <- GDP[Country %in% treatment]
holder <- transpose(as.data.frame(colSums(new_line[,2:95])))
names(holder) <- names(colSums(new_line[,2:95]))
setDT(holder)
new_line <- cbind(data.table(Country = "Treatment"), holder)
GDP <- rbind(GDP, new_line)


new_line <- Exports[Country %in% treatment]
holder <- transpose(as.data.frame(colSums(new_line[,2:81])))
names(holder) <- names(colSums(new_line[,2:81]))
setDT(holder)
new_line <- cbind(data.table(Country = "Treatment"), holder)
Exports <- rbind(Exports, new_line)

new_line <- Imports[Country %in% treatment]
holder <- transpose(as.data.frame(colSums(new_line[,2:81])))
names(holder) <- names(colSums(new_line[,2:81]))
setDT(holder)
new_line <- cbind(data.table(Country = "Treatment"), holder)
Imports <- rbind(Imports, new_line)

# new_line <- Population[Country %in% treatment]
# holder <- transpose(as.data.frame(colSums(new_line[,2:81])))
# names(holder) <- names(colSums(new_line[,2:81]))
# setDT(holder)
# new_line <- cbind(data.table(Country = "Treatment"), holder)
# Population <- rbind(Population, new_line)

setnames(gdp_change, old = "Real GDP growth (Annual percent change)", new = "Country")
new_line <- gdp_change[Country %in% treatment]
holder <- transpose(as.data.frame(colMeans(new_line[,2:43], na.rm = T)))
names(holder) <- names(colMeans(new_line[,2:43],na.rm = T))
setDT(holder)
new_line <- cbind(data.table(Country = "Treatment"), holder)
gdp_change <- rbind(gdp_change, new_line)
